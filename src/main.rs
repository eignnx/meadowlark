...
fn line_col(filename: &str, src: &str, pos: usize) -> (usize, usize) {
    let mut line = 1;
    let mut col = 1;
    ...
    panic!("pos {} out of range for {}", pos, filename);
}
    ...
    match e {
        ParseError::UnrecognizedToken {
            ...
            let expected = expected
                ...
                .collect::<Vec<_>>()
                .join(" or ");
            println!("Unexpected token [{path}:{line}:{col}]:");
        ...
        ParseError::InvalidToken { location } => {
            ...
            println!("Invalid token [{path}:{line}:{col}]");
        ...
        ParseError::UnrecognizedEof { location, expected } => {
            ...
            let expected = expected
                ...
                .collect::<Vec<_>>()
                .join(" or ");
            println!("Unexpected end of file [{path}:{line}:{col}]:");
